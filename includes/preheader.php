<?php
	...
	$MYSQL_HOST = HOST; //NIET localhost! mysqlstudent
	$MYSQL_LOGIN = USER; //thomasvandyeew3o
	$MYSQL_PASS = PASSWORD; //eaPhiu3gaexe
	$MYSQL_DB = DATABASE; // propositionbetting thomasvandyeew3o
    ...
    {
        function safeQuery($sqlString, $parameters = array())
        {
            global $mysqliConn;
        ...
        function createParameterTypeString($parameters)
        {
            ...
            for($i = 0; $i < count($parameters); $i++)
            {
                if(is_int($parameters[$i])) //Non numerical string === 0 = true
